import"./assets/modulepreload-polyfill-3cfb730f.js";import{a as p,i as E}from"./assets/sideBar-714322b6.js";import"./assets/vendor-b6f9ed32.js";const C="/js_98_Turg0n/assets/books-desk@1x-1f62c9bc.png",k="/js_98_Turg0n/assets/books-desk@2x-306b4264.png",S="/js_98_Turg0n/assets/books-mob@1x-93df6442.png",g="data:image/png;base64,",r="shoppingCard",a="shoppingList",t=document.querySelector(".js-shopping-list"),c=document.querySelector(".shopping-list-container"),T="The Florida governor gives his account of his achievements and shares his opinions about the political left. Read by John Pruden and the author. 8 hours, 59 minutes unabridged.",n=l(r),d=l(a);n[0]!==void 0?(c.innerHTML="",t.innerHTML="",t.insertAdjacentHTML("afterbegin",h(n)),t.addEventListener("click",v)):m();function v(e){let s=e.target.closest(".btn-shop");if(s)t.innerHTML="",n.splice(s.id,1),d.splice(s.id,1),localStorage.removeItem(r),localStorage.removeItem(a),A(r,n),A(a,d),n[0]!==void 0?t.insertAdjacentHTML("afterbegin",h(l(r))):m();else return}function h(e){return e.map(({book_image:s,title:o,author:u,description:i,amazonLinks:f,appleLinks:L},b)=>(i||(i=T),`<li class="shopping-list-item">
      <img class="shopping-list-card-img" src="${s}" alt="${o}">
      <div class="shopping-list-card-container">
        <h2 class="shopping-list-card-title">${o}</h2>
        <p class="shopping-list-card-text">Hardcover fiction</p>
        <p class="shopping-list-card-desc">${i}</p>
        <div class="shopping-list-card-bottom-container">
          <p class="shopping-list-card-author">${u}</p>
          <div class="shopping-list-card-container-img">
            <a href="${f}" class="shopping-amazon-link" rel="noopener noreferrer nofollow" target="_blank">
              <img class="shopping-list-card-icon-amazon"srcset="${p} 1x," src="${p}" alt="Amazon" loading="lazy">
            </a>
            <a href="${L}" class="shopping-book-link" rel="noopener noreferrer nofollow" target="_blank">
              <img class="shopping-list-card-icon-book" srcset="${g} 1x," src="${g}" alt="Apple books" loading="lazy">
            </a>
          </div>
        </div>
        <div>
          <button class="btn-shop shopping-list-card-container-trash" id="${b}">
            <svg class="shopping-list-card-icon-trash" width="18px" height="18px">
              <use href="${E}#icon-del"></use>
            </svg>
          </button>
        </div>
      </div>
    </li>
  `)).join("")}function m(){c.innerHTML="",c.insertAdjacentHTML("afterbegin",`
    <p class="shopping-list-text">
      This page is empty, add some books and proceed to order.
    </p>
    <img
      class="shopping-list-image"
      srcset="
        ${C} 1x,
        ${k} 2x
      "
      src="${S}"
      alt="books"
      width="265"
      height="198"
    />
    `)}function A(e,s){try{const o=JSON.stringify(s);localStorage.setItem(e,o)}catch(o){console.error("Get state error: ",o.message)}}function l(e){try{const s=localStorage.getItem(e);return s===null?void 0:JSON.parse(s)}catch(s){console.error("Get state error: ",s.message)}}
//# sourceMappingURL=commonHelpers3.js.map
